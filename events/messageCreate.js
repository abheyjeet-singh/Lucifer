    ...
    async execute(message, client) {
        ...
        if (autoResponders.length > 0) {
            ...
            for (const ar of autoResponders) {
                ...
                if (matched) {
                    if (ar.emoji) await message.react(ar.emoji).catch(() => {});
                    if (ar.response || ar.image_url) {
                        ...
                        if (ar.image_url) {
                            try {
                                ...
                                if (finalUrl.includes('media.tenor.com') && finalUrl.endsWith('.mp4')) finalUrl = finalUrl.replace('.mp4', '.gif');
                                ...
                                if (contentType.includes('text/html') || contentType.includes('application/json')) {
                                    ...
                                    let ext = finalUrl.match(/\.(gif|png|jpg|jpeg|webp|mp4)/i)?.[0]?.substring(1) || 'gif';
