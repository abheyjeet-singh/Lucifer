...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!target) return message.reply({ embeds: [createEmbed({ context: message, description: '⚠️ Mention a valid soul to damn eternally.', color: THEME.error })] });
    ...
    async run(client, guild, moderator, target, reason, context) {
        if (!target.bannable) return context.reply({ embeds: [createEmbed({ context: guild, description: '🚫 I cannot damn this soul. They may outrank me.', color: THEME.error })] });
        if (target.id === moderator.id) return context.reply({ embeds: [createEmbed({ context: guild, description: '🤔 Damning yourself to eternity? Dramatic.', color: THEME.secondary })] });
        ...
        try { await target.send({ embeds: [createEmbed({ context: guild, title: '🔥 Eternal Damnation', description: `You have been eternally banished from **${guild.name}**.\nNo mortal can undo this.\n**Reason:** ${reason}\n**Moderator:** ${moderator.user.tag}`, color: THEME.secondary })] }); } catch {}
