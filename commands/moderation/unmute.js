...
module.exports = {
    ...
    async run(client, guild, moderator, target, reason, context) {
        if (!target.isCommunicationDisabled()) return context.reply({ embeds: [createEmbed({ context: guild, description: '⚠️ That soul is not silenced.', color: THEME.error })] });
        ...
        return context.reply({ embeds: [createEmbed({ context: guild, description: `🔊 **${target.user.tag}** has been released from silence.`, color: THEME.success })] });
    },
};