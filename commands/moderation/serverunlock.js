...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        const msg = await message.reply({ embeds: [createEmbed({ context: message, description: '🔓 Unlocking the server...', color: THEME.accent })] });
        try {
            ...
            msg.edit({ embeds: [createEmbed({ context: guild, description: `🔓 **Server is now unlocked.**\n> ${reason}`, color: THEME.success })] });
        } catch { msg.edit({ embeds: [createEmbed({ context: guild, description: '🚫 Failed to unlock some channels.', color: THEME.error })] }); }
    },
    async interact(interaction, client) {
        const reason = interaction.options.getString('reason') || 'Server Unlock';
        await interaction.reply({ embeds: [createEmbed({ context: interaction, description: '🔓 Unlocking the server...', color: THEME.accent })] });
        try {
            ...
            interaction.editReply({ embeds: [createEmbed({ context: guild, description: `🔓 **Server is now unlocked.**\n> ${reason}`, color: THEME.success })] });
        } catch { interaction.editReply({ embeds: [createEmbed({ context: guild, description: '🚫 Failed to unlock some channels.', color: THEME.error })] }); }
    },
};