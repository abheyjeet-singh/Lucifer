...
module.exports = {
    ...
    async execute(message, args, client) { const target = message.mentions.members.first(); const ch = message.mentions.channels.first(); if (!target || !ch) return message.reply({ embeds: [createEmbed({ description: '⚠️ Use: `l!move @user #channel`', color: THEME.error })] }); return this.run(client, message.guild, message.member, target, ch, message); },
    ...
    async run(client, guild, moderator, target, channel, context) {
        ...
        return context.reply({ embeds: [createEmbed({ description: `⚡ **${target.user.tag}** has been teleported to ${channel}.`, color: THEME.primary })] });
    },
};
