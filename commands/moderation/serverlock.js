...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        const msg = await message.reply({ embeds: [createEmbed({ context: message, description: '🔒 Locking down the server...', color: THEME.accent })] });
        try {
            ...
            msg.edit({ embeds: [createEmbed({ context: guild, description: `🔒 **Server is now locked.**\n> ${reason}`, color: THEME.error })] });
        } catch { msg.edit({ embeds: [createEmbed({ context: guild, description: '🚫 Failed to lock some channels.', color: THEME.error })] }); }
    },
    async interact(interaction, client) {
        const reason = interaction.options.getString('reason') || 'Server Lockdown';
        await interaction.reply({ embeds: [createEmbed({ context: interaction, description: '🔒 Locking down the server...', color: THEME.accent })] });
        try {
            ...
            interaction.editReply({ embeds: [createEmbed({ context: guild, description: `🔒 **Server is now locked.**\n> ${reason}`, color: THEME.error })] });
        } catch { interaction.editReply({ embeds: [createEmbed({ context: guild, description: '🚫 Failed to lock some channels.', color: THEME.error })] }); }
    },
};
