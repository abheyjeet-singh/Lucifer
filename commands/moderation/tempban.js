...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!target) return message.reply({ embeds: [createEmbed({ context: message, description: '⚠️ Mention a valid soul to exile.', color: THEME.error })] });
    ...
    async run(client, guild, moderator, target, ms, durationStr, reason, context) {
        if (!target.bannable) return context.reply({ embeds: [createEmbed({ context: guild, description: '🚫 I cannot exile this soul.', color: THEME.error })] });
        if (isHardbanned(guild.id, target.id)) return context.reply({ embeds: [createEmbed({ context: guild, description: '🔥 That soul is eternally damned. A temporary exile is pointless.', color: THEME.error })] });
        ...
        try { await target.send({ embeds: [createEmbed({ context: guild, title: '⏳ Temporary Exile', description: `You have been exiled from **${guild.name}**\n**Duration:** ${durationStr}\n**Reason:** ${reason}\n**Return Time:** <t:${Math.floor(unbanTimestamp / 1000)}:R>`, color: THEME.secondary })] }); } catch {}
