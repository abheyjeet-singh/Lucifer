...
module.exports = {
    ...
    async run(client, guild, moderator, target, reason, context) {
        if (!target.bannable) return context.reply({ embeds: [createEmbed({ context: guild, description: '🚫 I cannot softban this soul.', color: THEME.error })] });
        ...
        return context.reply({ embeds: [createEmbed({ context: guild, description: `🧹 **${target.user.tag}** has been cleansed and returned.\n📋 **Reason:** ${reason}`, color: THEME.primary })] });
    },
};