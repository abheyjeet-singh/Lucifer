...
module.exports = {
    ...
    async execute(message, args, client) { const q = args.join(' '); if (!q) return message.reply({ embeds: [createEmbed({ context: message, description: '⚠️ Ask a question.', color: THEME.error })] }); return this.run(client, message, q); },
    ...
    async run(client, context, question) {
        const embed = createEmbed({ context: guild, title: '📊 Divine Poll', description: question, color: THEME.celestial });
