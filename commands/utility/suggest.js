...
module.exports = {
    ...
    async execute(message, args, client) { const idea = args.join(' '); if (!idea) return message.reply({ embeds: [createEmbed({ description: '⚠️ Provide an idea.', color: THEME.error })] }); return this.run(client, message.guild, message.author, idea, message); },
    ...
    async run(client, guild, user, idea, context) {
        const chId = getSuggestionChannel(guild.id); if (!chId) return context.reply({ embeds: [createEmbed({ description: '⚠️ Suggestions not configured.', color: THEME.error })] });
        const ch = guild.channels.cache.get(chId); if (!ch) return context.reply({ embeds: [createEmbed({ description: '⚠️ Suggestion channel missing.', color: THEME.error })] });
        const embed = createEmbed({ title: '🗳️ New Prayer', description: `**From:** ${user}\n\n${idea}`, color: THEME.celestial });
        ...
        return context.reply({ embeds: [createEmbed({ description: '🗳️ Your prayer has been submitted.', color: THEME.success })] });
    },
};
