...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!channel) return message.reply({ embeds: [createEmbed({ context: message, description: '⚠️ Mention a channel.', color: THEME.error })] });
    ...
    async show(client, guild, context) {
        ...
        if (!settings.log_channel_id) return context.reply({ embeds: [createEmbed({ context: guild, description: '📜 No hall of judgment has been set.', color: THEME.dark })] });
        const ch = guild.channels.cache.get(settings.log_channel_id);
        return context.reply({ embeds: [createEmbed({ context: guild, description: `📜 Hall of Judgment: ${ch || 'Unknown channel'}`, color: THEME.celestial })] });
    },

    async setCh(client, guild, member, channel, context) {
        if (!hasPermission(member, 'Administrator')) return context.reply({ embeds: [createEmbed({ context: guild, description: '🚫 Only administrators may set the log channel.', color: THEME.error })] });
        setLogChannel(guild.id, channel.id);
        return context.reply({ embeds: [createEmbed({ context: guild, description: `📜 Hall of Judgment set to ${channel}.`, color: THEME.success })] });
    },

    async off(client, guild, member, context) {
        if (!hasPermission(member, 'Administrator')) return context.reply({ embeds: [createEmbed({ context: guild, description: '🚫 Only administrators may disable logging.', color: THEME.error })] });
        removeLogChannel(guild.id);
        return context.reply({ embeds: [createEmbed({ context: guild, description: '📜 Hall of Judgment has been disbanded.', color: THEME.primary })] });
    },
};
