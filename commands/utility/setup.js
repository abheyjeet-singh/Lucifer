...
module.exports = {
    ...
    async execute(message, args, client) { return message.reply({ embeds: [createEmbed({ context: message, description: '⚠️ Use `/setup` slash commands.', color: THEME.error })] }); },
    async interact(interaction, client) {
        const sub = interaction.options.getSubcommand();
        // Removed starboard block
        if (sub === 'suggestions') { const ch = interaction.options.getChannel('channel'); setSuggestionChannel(interaction.guild.id, ch.id); return interaction.reply({ embeds: [createEmbed({ context: interaction, description: `🗳️ Suggestions set to ${ch}`, color: THEME.success })] }); }
        if (sub === 'counting') { const ch = interaction.options.getChannel('channel'); setCountingChannel(interaction.guild.id, ch.id); return interaction.reply({ embeds: [createEmbed({ context: interaction, description: `🔢 Counting set to ${ch}`, color: THEME.success })] }); }
        if (sub === 'sticky') { const ch = interaction.options.getChannel('channel'); const txt = interaction.options.getString('text'); if (txt.toLowerCase() === 'off') { removeSticky(ch.id); return interaction.reply({ embeds: [createEmbed({ context: interaction, description: `📌 Sticky removed from ${ch}`, color: THEME.accent })] }); } setSticky(ch.id, txt); return interaction.reply({ embeds: [createEmbed({ context: interaction, description: `📌 Sticky set in ${ch}`, color: THEME.success })] }); }
        if (sub === 'autodelete') { const ch = interaction.options.getChannel('channel'); const sec = interaction.options.getInteger('seconds'); if (sec === 0) { removeAutoDelete(ch.id); return interaction.reply({ embeds: [createEmbed({ context: interaction, description: `🗑️ Auto-delete disabled in ${ch}`, color: THEME.accent })] }); } setAutoDelete(ch.id, sec); return interaction.reply({ embeds: [createEmbed({ context: interaction, description: `🗑️ Auto-delete set to ${sec}s in ${ch}`, color: THEME.success })] }); }
    },
};